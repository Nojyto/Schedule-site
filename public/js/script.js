...
function updateTime(d, wd){
    ...
    if((timetable[8].e < d && wd == 5) || wd == 6 || wd == 7){
...
/*window.onload = () => {
    //test
    const d = new Date()
    updateTime(new Date(d.getFullYear(), d.getMonth(), 4, d.getHours() - 13, d.getMinutes() +32, d.getSeconds()).toLocaleTimeString("en-US", timeOptions), new Date().getDay() - 3)
    window.setInterval(() => {
        const d = new Date()
        updateTime(new Date(d.getFullYear(), d.getMonth(), 4, d.getHours() - 13, d.getMinutes() + 32, d.getSeconds()).toLocaleTimeString("en-US", timeOptions), new Date().getDay() - 3)
    }, 1000)
